...
it('exports customer ledger reports with metadata rows before ledger rows', function (): void {
    ...
    expect($contents)
        ...
        ->toContain("->visible(fn (RelationManager \$livewire): bool => \$livewire->hasPaidSalesForExport())")
        ->toContain('Some paid sales exist for this customer. Enable this to include them as reference rows without changing ledger balances.')
...
it('streams ledger rows instead of materializing the entire ledger in memory', function (): void {
    ...
    expect($contents)
        ...
        ->toContain('protected function hasPaidSalesForExport(): bool')
